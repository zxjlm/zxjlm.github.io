...
我们也可以跟踪容器日志的某一片段，和之前类似，只需要在 tail 命令后加入-f --tail 标志即可。例如，可以用 _docker logs --tail 10 daemon_dave_ 获取日志的最后 10 行内容。另外，也可以用_docker logs --tail 0 -f daemon_dave_ 命令来跟踪某个容器的最新日志而不必读取整个日志文件。
