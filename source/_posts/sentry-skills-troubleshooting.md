...
这次实验的重点不是 Grafana。关键证据全部来自 Sentry Skills 对 GlitchTip 事件的读取，因此这里记录一套通过 Sentry Skills 独立完成错误排查的方法。Grafana MCP 的使用可以参考 {% post_link grafana-mcp-usage '用 Grafana MCP 排查 Redis 配置问题与迭代监控' %}。
